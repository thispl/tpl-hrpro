...
def execute(filters=None):
    ...
    for ss in salary_slips:
        row = [ss.name, ss.employee, ss.employee_name, doj_map.get(ss.employee), ss.branch, ss.department, ss.designation,
            ss.company, ss.start_date, ss.end_date, ss.leave_without_pay, ss.payment_days,ss.total_working_hours]
...
def get_columns(salary_slips):
    ...
    columns = [
        ...
        _("End Date") + "::80", _("Leave Without Pay") + ":Float:130", _("Payment Days") + ":Float:120", _("Total Working Hours") + ":Float:120"
    ]
    """
    columns = [
        _("Salary Slip ID") + ":Link/Salary Slip:150",_("Employee") + ":Link/Employee:120", _("Employee Name") + "::140",
        ...
        _("End Date") + "::80", _("Leave Without Pay") + ":Float:-1", _("Payment Days") + ":Float:120" , _("Total Working Hours") + ":Float:120"
    ]
